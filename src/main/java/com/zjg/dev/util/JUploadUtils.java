...
public class JUploadUtils {
    ...
    public static File save(MultipartFile file, HttpServletRequest request) throws Exception {
        // 获取本地存储路径
        String path = request.getSession().getServletContext().getRealPath(Constants.FILEUPLOAD);
        String fileName = file.getOriginalFilename();
        // 取得后缀
        String suffixString = fileName.substring(fileName.lastIndexOf(".") + 1);
        String ip = IPUitls.getRemoteAddr(request);
    ...
    public static File save(String fileType, MultipartFile file, HttpServletRequest request) throws Exception {
        // 获取本地存储路径
        String path = request.getSession().getServletContext().getRealPath(Constants.FILEUPLOAD) + File.separator + fileType;
        String fileName = file.getOriginalFilename();
        // 取得后缀
